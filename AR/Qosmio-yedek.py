...
while True:
    ...
    # img = cv2.imread('Faces\\face10.jpg', 1)
    # ret = img
    ...
    try:
        ...
        cv2.circle(img_copy, eye_left, 2, color=(0, 255, 0), thickness=-5)
        cv2.circle(img_copy, eye_right, 2, color=(255, 0, 0), thickness=-5)
        cv2.imshow('Output 01',img_copy)
        ...
        print(resize_rate)
        print("-----------------------------")
        # face_width = -eye_left[1] + eye_right[1]
        # face_width = -eye_left_posx[1] + eye_right_posx[1]
        # print(type(w)) // w değişkeninin type

        # resize_glasses
        # glasses_resize = resize(glasses, face_width)
        # Fonksiyon "resized_pixel_locations" fonksiyonunun içine alınmıştır.

        # glasses_resize_rotated_centerX
        # grrc_x1, grrc_y1, glasses_resize, earLetfX, earLetfY, earRightX, earRightY = resized_pixel_locations(glasses, glassesCenterX, glassesCenterY, face_width, degree+90, earLetfX, earLetfY, earRightX, earRightY)
        # grrc_x1, grrc_y1, glasses_resize = resized_pixel_locations(glasses, glassesCenterX, glassesCenterY, face_width, degree+90)
        ...
        # grrc_x1, grrc_y1, glasses_resize = resized_pixel_locations(glasses, mtr[head_degree4file][0], mtr[head_degree4file][1], resize_rate, degree + 90) # Sol göze göre yerleştirme

        # grc_x0, grc_y0 glasses_resized_coordinates
        # grrc_x1, grrc_y1 glasses_resized_rotated_coordinates
        ...
        # glass_rec_rotated = ndimage.rotate(img[y + glass_trans + differenceY : y + yG + glass_trans + differenceY, x + differenceX : w + differenceX], (degree+90))
        # glass_rec_rotated = ndimage.rotate(img[ nose[0] - grrc_x1 : nose[0] - grrc_x1 + xG, nose[1] - grrc_y1 : nose[1] - grrc_y1 + yG ], (degree+90))
        ...
        # glass_rec_rotated = ndimage.rotate(img[eye_left[1] - grrc_y1: eye_left[1] - grrc_y1 + yG, eye_left[0] - grrc_x1: eye_left[0] - grrc_x1 + xG], (degree + 90)) # Sol göze göre yerleştirme
        ...
        # rec_resize = img_copy[y + glass_trans + differenceY : y + h5 + glass_trans + differenceY, x + differenceX : x + w5 + differenceX]
        # rec_resize = img_copy[ nose[0] - grrc_x1 : nose[0] - grrc_x1 + w5, nose[1] - grrc_y1 : nose[1] - grrc_y1 + h5 ]
        rec_resize = img_copy[nose[1] - grrc_y1 : nose[1] - grrc_y1 + h5, nose[0] - grrc_x1 : nose[0] - grrc_x1 + w5] # Buruna göre yerleştirme
        # rec_resize = img_copy[eye_left[1] - grrc_y1 : eye_left[1] - grrc_y1 + h5, eye_left[0] - grrc_x1 : eye_left[0] - grrc_x1 + w5] # Sol göze göre yerleştirme
        ...
        # img_copy[y + glass_trans + differenceY : y + h5 + glass_trans + differenceY, x + differenceX : x + w5 + differenceX] = blend_glass3
        # img_copy[nose[0] - grrc_x1: nose[0] - grrc_x1 + w5, nose[1] - grrc_y1: nose[1] - grrc_y1 + h5] = blend_glass3
        img_copy[nose[1] - grrc_y1: nose[1] - grrc_y1 + h5, nose[0] - grrc_x1: nose[0] - grrc_x1 + w5] = blend_glass3 # Buruna göre yerleştirme
        # img_copy[eye_left[1] - grrc_y1: eye_left[1] - grrc_y1 + h5, eye_left[0] - grrc_x1: eye_left[0] - grrc_x1 + w5] = blend_glass3 # Sol göze göre yerleştirme
        ...
        if head_degree > 5:
            ...
            ear4degree = np.rad2deg(np.arctan2(left[0] - eye_left[0], left[1] - eye_left[1]))

            # Yüzdeki koordinatlar ve saptaki koordinatlardan yararlanılarak sapın resize edileceği oranın bulunması ve uygulanması
            earResizeRateX = (eye_left[0] - left[0]) / (-ear_left_file[0] + ear_eye_left_file[0])
            ...
            eye_ear_img_left = img_copy[ eye_left[1] - ear_eye_left[1]: eye_left[1] - ear_eye_left[1] + ear_img_left.shape[0], eye_left[0] - ear_eye_left[0]: eye_left[0] - ear_eye_left[0] + ear_img_left.shape[1]]
            ear_img_left = blend_transparent(eye_ear_img_left, ear_img_left)
            img_copy[eye_left[1] - ear_eye_left[1]: eye_left[1] - ear_eye_left[1] + ear_img_left.shape[0], eye_left[0] - ear_eye_left[0]: eye_left[0] - ear_eye_left[0] + ear_img_left.shape[1]] = ear_img_left
            ...
            ear4degree = np.rad2deg(np.arctan2(-right[0] + eye_right[0], -right[1] + eye_right[1]))

            # Yüzdeki koordinatlar ve saptaki koordinatlardan yararlanılarak sapın resize edileceği oranın bulunması ve uygulanması
            earResizeRateX = (eye_right[0] - right[0]) / (ear_right_file[0] - ear_eye_right_file[0])
            # if earResizeRateX < 0:
            #     earResizeRateX = -earResizeRateX
            ...
            eye_ear_img_right = img_copy[eye_right[1] - ear_right[1]: eye_right[1] - ear_right[1] + ear_img_right.shape[0], eye_right[0] - ear_right[0]: eye_right[0] - ear_right[0] + ear_img_right.shape[1]]
            ear_img_right = blend_transparent(eye_ear_img_right, ear_img_right)
            img_copy[eye_right[1] - ear_right[1]: eye_right[1] - ear_right[1] + ear_img_right.shape[0], eye_right[0] - ear_right[0]: eye_right[0] - ear_right[0] + ear_img_right.shape[1]] = ear_img_right

            # -------------------------------------------------------------------------------
            # -------------------------------------------------------------------------------

        cv2.imshow('Output', img_copy)
        cv2.imwrite('Save0.png', img_copy)
